<template>
  <div>
    <div class="container__cards">
      ...
      <div
        v-else
        @click="requestRedirect(el.link)"
        v-for="(el, id) in dataArr"
        :key="id"
        class="card__element"
      >
        <div class="el__text">
...
.container__cards
  ...
  .card__element
    ...
    max-width: 290px
    padding: 20px
    background: radial-gradient(50% 50% at 50% 50%, rgba(89, 89, 89, 0.2) 0%, rgba(0, 0, 0, 0) 100%), #171717
    border-radius: 32px
    cursor: pointer
    &:nth-child(2) .el__text
      max-width: 55px
      text-align: center
    &:nth-child(4) .el__text
      max-width: 122px
    &:nth-child(5) .el__text
      max-width: 100px
    &:nth-child(7) .el__text
      max-width: 70px
    &:nth-child(8) .el__text
      max-width: 120px
      margin-bottom: 10%
